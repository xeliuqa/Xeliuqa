...
public class btc_send extends AppCompatActivity {
    ...
    private String api_url = "http://"+ conf.url()+":8081/v1/transaction/btc";
    private String api_url_raw = "http://"+conf.url()+":8081/v1/push/btc";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_btc_send);
        ...
        logo.setBackgroundResource(R.drawable.btc);
        ...
        back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(btc_send.this, Transaction_BTC.class);
                startActivity(i);
            }
        });
                ...
                if (to.isEmpty() || to.length() == 0 || to.equals("") || to == null || amt.isEmpty() || amt.length() == 0 || amt.equals("") || amt == null) {
                    ...
                    if (toamount > mybals){
                        ...
                        if (otp_status.equals("1")){
                            ...
                            LayoutInflater li = LayoutInflater.from(btc_send.this);
                            View promptsView = li.inflate(R.layout.activity_otp, null);

                            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(btc_send.this);
                                            ...
                                            new DialogInterface.OnClickListener() {
                                                public void onClick(DialogInterface dialog,int id) {
                                                    ...
                                                    if (otp.equals(OTPs)){
                                                        ...
                                                        alert.showDialog(btc_send.this, "OTP is wrong.");
                                                    }
                                                }
                                            })
    ...
    private void displayLoader() {
        pDialog = new ProgressDialog(btc_send.this);
    ...
    private void send_raw()
    {
        ...
        try {
            //Populate the request parameters
            request.put("raw", rawTransaction);

        } catch (JSONException e) {
            //e.printStackTrace();
        }
        JsonObjectRequest jsArrayRequest = new JsonObjectRequest
                (com.android.volley.Request.Method.POST, api_url_raw, request, new com.android.volley.Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        pDialog.dismiss();
                        try {
                            if (response.getString(KEY_STATUS) == "false") {
                                String res = response.getString(KEY_MESSAGE);
                                ...
                                Intent i = new Intent(getApplicationContext(), blockchain.class);
                                i.putExtra("hash", part3);
                                i.putExtra("coin","BTC");
                                startActivity(i);

                                rawTransaction="";
                            }else if(response.getString(KEY_STATUS) == "true"){
                                String call = response.getString(KEY_MESSAGE);
                                if (call.equals("undefined")){
